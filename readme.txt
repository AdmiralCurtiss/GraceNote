GraceNote v10.0 ~Lite~

---- Execution ----
Run GraceNote.py using Python 2.7.x.
By default, config.xml in the same folder as GraceNote.py is used for settings, FTP- and Database-Paths. You can load another config file by giving it to GraceNote.py as a command line parameter.
...
====Manual====
No changes to the status of entries will happen in this mode unless the status icons in each entry are explicitly clicked.
